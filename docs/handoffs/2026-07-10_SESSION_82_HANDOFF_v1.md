...
- Enhanced Current Classes section with student lists + emails.
- Game Topics section with delete capability.
- Mode preference tallies in overview.
- Class request approval shows requested class name.
- Teacher table shows mode preferences.
...
- **Groovr Development Story** — narrative documentary of how the app evolved across ~82 sessions.
- **Groovr Technical Blueprint** — procedure document: file structure, schema, setup, and how to rebuild.
...
Mike is testing all admin/teacher interactions. No code deliverables pending — this is manual verification.
...
7. **Expanded grid sizes: 4×4 (16 students) and 5×5 (25 students).** (NEW — session 82) The grid rendering already uses dynamic `gridTemplateColumns`, so the visual side scales. Changes needed: allow 16 and 25 as valid `classes.capacity` values; add a "requested capacity" field to the class request form (dropdown: 9/16/25); admin approval honors or overrides the requested capacity. The game engine (voting, favorites, rounds) iterates over entries, not grid cells, so it should scale without changes — but verify nothing in `/play` hardcodes a 3×3 assumption. **Moderate lift.** Mostly UI changes to request form and admin approval. Worth verifying `src/app/play/` files before committing.
8. **Teacher requests class size instead of mode willingness.** (NEW — session 82) Reframe: instead of "willing for trio/nine," teacher requests "I want a class of 9/16/25." Admin grants or assigns differently. This would replace `willing_trio`/`willing_nine` on `teacher_rotation` with `requested_capacity` on `class_requests`. The warmup teacher count (1/3/9) remains a separate admin-level setting. **Slight schema change, simpler mental model.** Caution: the `willing_trio`/`willing_nine` columns were just added in session 81. If this replaces them, that migration was premature. Consider whether mode willingness and class size are truly independent axes before dropping the columns.
9. **Teacher game / teacher competition.** (NEW — session 82) Teachers play the same game loop as students: each teacher uploads 2–3 photos, they fill a grid, teachers vote/comment/pick favorites, then a teacher awards ceremony. Natural grid pairings: 3 teachers × 3 photos = 3×3, 4 teachers × 4 photos = 4×4, 5 teachers × 5 photos = 5×5. Or: teachers upload min 2 / max 3 and the grid fills dynamically (more flexible, harder to design around). **This is scope creep — but well-bounded scope creep** because the entire game engine already exists. Implementation: reuse `entries`/`game_sessions`/`games` tables with a `game_type = 'teacher'` flag (or a parallel `teacher_games` table). Different enrollment logic (all teachers auto-enrolled vs. student opt-in). **Recommendation: defer until student game is solid at all grid sizes.** The teacher game is essentially "run the same game with different players" and should be a clean add-on once the engine handles 9/16/25 grids. Capture the design clearly now; build later.
...
```sql
-- Session 81 Chunk 1.5 migration (already run)
...
No new schema changes in session 82.
...
| `teacher_rotation` | teacher_id (unique), sort_order, status, willing_trio, willing_nine | status: recruiting/waiting/paused. Mode prefs session 81. |
...
| `class_requests` | id, teacher_id, status, class_name, requested_at, reviewed_by, reviewed_at, created_class_id | status: pending/approved/denied. class_name session 81. |
...
**To test class requests:** Log in as a teacher. Click "Request new class." Fill in class name + mode preferences. Submit. Log in as admin and verify the request appears with the class name. Approve it. Teacher should see the new class in their dropdown.

**To test admin dashboard:** Log in as admin. Verify merged teacher table with ↑↓ buttons. Expand/collapse teachers section. Check class details show students + emails. Verify game topics list shows approved topics with delete option. Check mode preference tallies in overview.
