...
### Priority 1: Multi-Teacher Game — Phase 4 (Browse Page + Student Play)

Students discover and play multi-teacher games. This is the next major build.

**What needs to happen:**
- Add multi-teacher game cards to the `/play` browse page alongside regular class listings
- Build `/play/multi/[gameId]` — student play page with 3×3 photo grid (blind, no avatars), shuffle-stop through all 9, then pick top 3 favorites with avatar reveal + comment on each
...
- Create `multi_game_comments` table (for favorite-linked comments)

**Files to request at session start:**
- `src/app/play/page.tsx` — current browse page (to add multi-teacher game cards)
- `src/game/spotlight.jsx` — photo grid rendering, shuffle-stop mechanics, comment flow
- `src/game/monsters.jsx` — avatar SVGs for the reveal moments
- `src/app/play/actions.ts` — existing enrollment/magic-link patterns
- `MULTI_TEACHER_GAME_DESIGN.md` — Phase 4 section for full spec

**Design reference:** See Phase 4 in the design document. Key details:
- 3×3 grid of 9 photos (3 teachers × 3 photos), completely blind — no avatars, no names
- Shuffle-stop through all 9 with comments (same flow as existing warmup)
- Then pick top 3 favorites in sequence: 1st (3pts) → 2nd (2pts) → 3rd (1pt)
- Each pick reveals the monster avatar of the teacher who posted that photo
- Student joins via email (magic link), no full profile needed

### Priority 2: Multi-Teacher Game — Phases 5–8

After student play works:
- **Phase 5** — Comment review + teacher feedback (teacher reviews favorite-linked comments, approve/reject for reveal, private replies, student flagging)
- **Phase 6** — Reveal ceremony (personalized per-student two-layer reveal: avatar → real teacher, approved comments shown)
...
- [x] **Drop `teacher_rotation` table** — done Session 104 migration
- [x] **Drop stale DB columns** — `selected_solo`, `selected_trio`, `selected_full` on entries — done Session 104 migration
- [ ] **Drop dead files** — `switch-mode-footer.tsx`, `request-actions.ts` (no longer imported). Just delete them from the repo.
...
| 5 | Comment Review + Teacher Feedback — approve/reject favorite comments, replies, flagging | Not started |
| 6 | Reveal Ceremony — personalized two-layer reveal per student | Not started |
...
**Database tables created (7 of 11):**
- ✅ `multi_teacher_games` — the game event (now has `avatar_mode` column)
...
- `multi_game_comments` — private student-teacher comments with approval workflow
...
- **Multi-teacher game detail:** `/teacher/multi/[gameId]` — max-width 720px layout (Session 104), round tabs, avatar spinner gate (spin to discover your monster before uploading), per-round photo upload (3 per teacher), avatar badge with expandable profile (Session 104), collapsible topic editing (Session 104), readiness tracking, start game controls. Spin blocked until previous round has photos (Session 104). Game settings editable by creator until another teacher joins (Session 104).
...
- `multi/actions.ts` — `createMultiGame`, `joinMultiGame`, `leaveMultiGame`, `uploadMultiPhoto`, `deleteMultiPhoto`, `startMultiGame`, `deleteMultiGame`, `updateRoundTopic`, `updateGameSettings` (locked once 2nd teacher joins, Session 104), `spinAvatar` (prev-round photo gate, Session 104)
- `multi/[gameId]/page.tsx` — Game detail server component. Fetches monsters + avatar assignments for spinner. Max-width 720px wrapper (Session 104).
- `multi/[gameId]/game-detail-client.tsx` — Round tabs, avatar spinner gate (spin before upload), avatar badge with expandable profile (Session 104), collapsible topic section (Session 104), per-round photo upload (3 per teacher), spin blocked until prev round has photos (Session 104), game settings editor for solo creator (Session 104), readiness tracking, start game controls
- `multi/[gameId]/avatar-spinner.tsx` — 3×3 monster grid with shuffle-stop mechanic. Handles single/rotating avatar modes, greyed-out used monsters, reveal + confirm flow.
...
## Screenshot Notes (Session 104)

- **Screenshot 1 (Supabase SQL editor):** monster_avatars migration error "policy already exists" — the migration was run a second time. Not a bug; just means the `CREATE TABLE IF NOT EXISTS` passed but the `CREATE POLICY` failed because it already existed. No action needed.
- **Dead files to delete from repo:** `switch-mode-footer.tsx` and `request-actions.ts` — find and delete them manually. They're not imported by anything.
