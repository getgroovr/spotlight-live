...
- **Multi-teacher game detail:** `/teacher/multi/[gameId]` — round tabs, avatar spinner gate (spin to discover your monster before uploading), per-round photo upload (3 per teacher), avatar badge showing assigned monster, topic editing (creator only), readiness tracking, start game controls.
...
- `multi/actions.ts` — `createMultiGame`, `joinMultiGame`, `leaveMultiGame`, `uploadMultiPhoto`, `deleteMultiPhoto`, `startMultiGame`, `deleteMultiGame`, `updateRoundTopic`, `updateGameSettings`, `spinAvatar`
- `multi/[gameId]/page.tsx` — Game detail server component. Fetches monsters + avatar assignments for spinner.
- `multi/[gameId]/game-detail-client.tsx` — Round tabs, avatar spinner gate (spin before upload), avatar badge, per-round photo upload (3 per teacher), topic editing (creator only), readiness tracking, start game controls
