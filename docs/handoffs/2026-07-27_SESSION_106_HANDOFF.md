...
- **Teacherhood ladder** (student winner → teacher candidate) — interesting growth concept.
- **Open enrollment / Front Door** — superseded by the browse page concept.
...
**Database tables created (7 + review columns + lifecycle columns):**
- ✅ `multi_teacher_games` — the game event (has `avatar_mode`, `archive_after`, `replay_of` columns)
- ✅ `multi_teacher_participants` — teachers in a game (has `participant_status` column: active/waiting)
...
**Game lifecycle:** `forming` → `active` → `complete` (with `archived` for auto-archived games)
...
- `multi/page.tsx` — Multi-teacher game lobby server component. Calls `checkAutoArchive` on load (Session 106). Fetches `participant_status` for waiting list (Session 106).
- `multi/multi-client.tsx` — Lobby client: My Games (with "Manage →" links), Available Games, Create Game form (collapsible), Game History section (Session 106). Max-width 720px layout. Waiting badge, drop out button (Session 106).
- `multi/actions.ts` — `createMultiGame`, `joinMultiGame` (with waiting list cap, Session 106), `leaveMultiGame`, `uploadMultiPhoto`, `deleteMultiPhoto`, `startMultiGame`, `deleteMultiGame`, `updateRoundTopic`, `updateGameSettings`, `spinAvatar`, `approveComment` (Session 105), `rejectComment` (Session 105), `replyToComment` (Session 105), `flagStudent` (Session 105), `dropOutOfGame` (Session 106), `completeGame` (Session 106), `replayGame` (Session 106)
- `multi/[gameId]/page.tsx` — Game detail server component. Fetches monsters + avatar assignments for spinner. Fetches favorite comments for review inbox (Session 105). Fetches game stats + waiting list (Session 106). Max-width 720px wrapper.
- `multi/[gameId]/game-detail-client.tsx` — Round tabs, avatar spinner gate, avatar badge with expandable profile, collapsible topic section, per-round photo upload (3 per teacher), readiness tracking, start game controls. CommentInbox + CommentCard components (Session 105). StatsSection with medal rankings + per-teacher breakdowns, CompleteGameSection, ReplaySection, DropOutSection, WaitingListSection (Session 106).
...
- `multi-game-lifecycle.ts` — `checkAutoArchive`, `promoteWaiter`, `countActiveParticipants`, `buildReplayGame` (Session 106)
- `multi-game-stats.ts` — `getGameStats`: per-teacher vote tallies, pick breakdowns, comment counts (Session 106)
...
14. `20260727120000_multi_game_lifecycle.sql` — Waiting list (participant_status), auto-archive (archive_after), replay link (replay_of)
...
| S106-lifecycle | 106 | Phase 7: Waiting list (participant_status, cap at 3 active, promote waiters), dropOutOfGame, completeGame, replayGame actions, auto-archive on lobby load, 7-day archive deadline. Phase 8: getGameStats aggregation (per-teacher vote tallies, pick breakdowns, comment counts), StatsSection with medal rankings, game history section in lobby, lobby wired with lifecycle + stats. Migration: participant_status, archive_after, replay_of columns. |
