...
This is a design/UX task that touches multiple files. The data layer work should be aware of it but the full timer UI is deferred.
...
## KEY CONCEPT: Awards / Results Voting Rules (clarified session 61)

**MIN_VOTES gates the ROUND, not individual entries.**

- A round needs at least one entry with ≥2 favorite votes to have any winners.
- If nobody in the round gets 2+ votes → `noWinners = true` → UI shows "No clear favorite this round."
- Once the round qualifies (gold has 2+ votes). (deletion here by Mike).  No metals to be awarded below 
two favorites- 2 being the minimum requirement to qualify for any metal.
- Competition ranking: ties share a rank, next rank skipped. Two golds → no silver → next is bronze (rank 3).
- No 9-way tie of golds is possible — a round where everyone got 1 vote triggers noWinners.

---

## What got done this session (61)

### Files delivered (all include previous session 60 edits + new fixes)

| File | Destination | What changed |
|---|---|---|
| `shell.jsx` | `src/game/shell.jsx` | **B61:** StrictMode splash fix — `markRoundSplashSeen` moved from the show-effect to dismiss callbacks only. In dev, StrictMode double-fires effects; the old code marked the splash as seen immediately, so the second pass skipped it (appeared for one frame, vanished). Now marks seen only on timer expiry or tap. Also: `Number()` cast on `currentRound` for Supabase string safety. |
| `spotlight.jsx` | `src/game/spotlight.jsx` | **Back button:** ← in game header navigates to `/student/dashboard` for student mode (was `setView("splash")` which auto-skipped back to game via B35). **Favorite text:** richer pre-save instructions ("Write what you liked about this photo — your classmates may see your comment during the awards. Feel free to edit it until you're happy, then hit Save."). **Upload preview:** file input now shows a photo preview via `URL.createObjectURL` when the student picks a file, so they can see what they're uploading. |
| `page_-_dashboard.tsx` | `src/app/student/dashboard/page.tsx` | **B60:** Round-complete gate — green "✓ Round N complete — sit tight until the next round!" banner replaces the play button when `sessionByRound.has(currentRound)`. **B54/B61:** Played rounds condense — if the student has a game_session for the current round, it moves to the "Completed rounds" band (collapsed `<details>`) instead of showing as the full "CURRENT ROUND — LIVE NOW" card. |
| `game-results.ts` | `src/lib/game-results.ts` | **MIN_VOTES fix:** threshold now gates the round (top entry must have ≥2 votes), not individual entries. Silver/bronze with 1 vote now appear when gold has 2+. |
| `page_-_results.tsx` | `src/app/student/results/page.tsx` | **Layout tightened:** gold photo 220→160px, silver/bronze 140→100px, trophy 68→52, title 34→28, container 640→520px, card padding/margins reduced. |

### Bugs closed this session (61)
...
| — | Results only show 1st place | MIN_VOTES gates round, not individual entries |
| — | Results page too large | Photo sizes, padding, fonts all tightened |
...
|---|---|---|---|
| B41 | No photo preview in teacher's review card for picture submittals (SS1 from session 61 testing) | MEDIUM | Open |
| B62 | Teacher student detail page: some photos not loading — may be seed data placeholder URLs (`placeholdit.co`) that don't resolve, or signing issues | MEDIUM | Open — need teacher page code |
...
### Notes on photo issues
Mike has reported missing photos across multiple sessions. Three distinct issues:
1. **Teacher submittal thumbnails (B41):** "no preview" in the teacher review queue. Needs the teacher page code (`src/app/teacher/students/page.tsx`) to diagnose — likely a missing signed URL or wrong bucket reference.
2. **Seed data placeholders (B62):** Test seed data uses `https://placeholdit.co/...` URLs that may not resolve. Fix by using the Supabase `seed-photos` bucket URLs instead. Needs updated seed SQL.
3. **Awards page photos:** Some entries show the placeholder "R3" image — this IS the correct image when the seed voter's entry was created with a placeholder URL. Not a bug per se, but looks broken during testing.
...
7. **Round gap timing UI** — student-facing countdown showing submission deadline vs round boundary; teacher-facing review window indicator. Schema (`review_time_hours`) lands in next data layer session; UI deferred.
...
## NEXT SESSION (62): Testing + Teacher Photo Fix + Data Layer

Mike will run a full testing walkthrough to verify session 61 fixes (especially the splash). Reserve time for any new issues.

### Priority order
1. **Verify splash works** — if still broken after StrictMode fix, the issue is deeper (possibly `computeCurrentRound` returning 0 during certain timing states). Will need `round-timing.ts` debugging.
2. **Fix teacher page missing photos (B41/B62)** — need `src/app/teacher/students/page.tsx` and related components
3. **Data layer: thread `game_id` through all queries** (carried from session 60 plan)
4. **Teacher game lifecycle: start/end controls** (carried from session 60 plan)

### Chunk 1: Teacher photo fixes

**Files needed:**
1. This handoff
2. `src/app/teacher/students/page.tsx` — teacher dashboard (the review queue with "no preview")
3. Any component that renders the submittal review cards (if separate from page.tsx)
4. `src/app/teacher/students/actions.ts` — teacher server actions

**Tasks:**
- Diagnose why submittal thumbnails show "no preview" (B41)
- Fix signed URL generation for submitted photos in the teacher review queue
- Verify teacher student detail page photos load correctly

### Chunk 2: Data layer — game_id awareness (carried from session 60)

**Files needed:**
1. This handoff
2. `src/lib/class-deck.ts`
3. `src/lib/game-results.ts`
4. `src/lib/student-archive.ts`
5. `src/app/play/actions.ts`

**Tasks:**
Same as session 60 handoff Chunk 1 — thread `game_id` through all queries and writes. Add `review_time_hours` + `games.started_at` migration. Add `resolveActiveGame()` helper.

### Chunk 3: Testing bugs

**Files needed:** Whatever Mike's walkthrough surfaces.
...
**IMPORTANT: Run the appropriate jump-to SQL BETWEEN rounds during testing.** The jump-to files simulate seed voter sessions for each round. Without running them, seed voters won't have sessions/votes for that round, and the awards page will have minimal data.
...
- ✅ Session 61: Testing fixes (splash StrictMode, round gate, condensing, results silver/bronze, layout)
- 🔲 Session 62: Teacher photo fixes + data layer game_id awareness + teacher game lifecycle
- 🔲 Session 63: Round gap timing UI (review_time_hours), student-facing countdown timers
- 🔲 Session 64: Multi-class polish, remaining bugs (B49, B11)

### Architecture (unchanged)

**`games` table (in place, session 62 additions noted):**
```
id            uuid PK
...
started_at    timestamptz (NEW — set when teacher starts game)
created_at    timestamptz
```

**`classes` table addition (session 62):**
```
